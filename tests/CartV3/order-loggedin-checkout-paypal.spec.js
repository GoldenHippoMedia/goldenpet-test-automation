...
// GI: "Order - Log In, Add a Standard product, From the Checkout page,
//      Check out using PayPal (EXCLUDE PROD) (Mike)"
//
// Flow: Login → Cart → click "change" link → /checkout → Click PayPal button
//       → PayPal popup login → Popup closes → checkout auto-submits → Confirmation
//
...
test.describe('Order - Logged-In Checkout with PayPal', () => {
  // Generous timeout — PayPal popup + upsell funnel can take 60–90s
  test.setTimeout(180000);

  test(
    'logged-in order: PayPal from /checkout and validate confirmation',
    { tag: '@real-order' },
    async ({ page, brand }) => {
      ...
      const productKey = Math.random() < 0.5 ? 'loggedin_std_3' : 'loggedin_std_4';
      ...
      //    (same path used by the saved-CC and PayPal-button cart tests)
      await cartPage.changeShippingLink.click();
      await page.waitForURL(/\/checkout/, { timeout: 15000, waitUntil: 'commit' });

      // 5. Wait for /checkout to be interactive — PayPal button container should render
      await page.locator('#paypal-button').waitFor({ state: 'visible', timeout: 30000 });

      // 6. Snapshot checkout summary
      const checkoutSnap = await checkoutPage.getOrderSummary();
      assertSnapshotsAgree(cartSnap, checkoutSnap, 'cart', 'checkout', ['subtotal']);

      // 7. Click PayPal button on /checkout and complete auth in the popup.
      ...
      // --- PayPal sandbox login flow (same pattern as the other PayPal tests) ---
      ...
      // 8. After PayPal closes, /checkout auto-submits the order (same behavior as
      //    the logged-in cart PayPal flow — no manual Submit Order click required).
      //    Page navigates /checkout → /upsell → /order-confirmation.
      ...
      console.log(`  Payment:      PayPal (logged-in /checkout)`);
      ...
      test.info().annotations.push(
        ...
        { type: 'Payment Method',  description: 'PayPal (logged-in /checkout)' },
