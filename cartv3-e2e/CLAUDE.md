...
- **✅ RESOLVED 2026-08-04 — the two blockers are now SEPARATED by hard evidence (read the Turnstile sitekey straight out of the page config). This supersedes the guesswork in the entries below.** The app ships its client config inline in the page HTML, so the Turnstile sitekey is directly observable — on `/checkout` or `/payment-details`, in DevTools console:
  ...
  **Conclusions (both earlier theories were right, but in different envs):**
  ...
  - **UAT is NOT Turnstile-blocked** (test key auto-passes), so every UAT "Too many requests" / stuck-on-`/checkout` failure was the **CF velocity rate-limit** — the `401 /account-service/proxy/auth/me` + `419 /proxy/funnel/stats/save/` cf-ray responses. DevOps's "it's most likely Turnstile" was correct for prod but **wrong for UAT**.
  - **CART-9181 disposition:** the *prod* half is not actionable (live Turnstile is intended security — do not ask for a bypass). The *UAT* half is the rate-limit exception ask (keyed on the `DrMartyQA/<token>` UA, NOT IP) and is still legitimately open — currently intermittent/non-fatal (a dense 7-spec `tests/order-*.spec.js` batch on 2026-08-04 placed 5 real orders and `order-loggedin-checkout-cc` PASSED, with the cf-ray hits present but non-blocking).
